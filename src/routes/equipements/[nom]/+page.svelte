<script lang="ts">
	// ===============================================================
	// Objectif : afficher la fiche détaillée d’un équipement individuel
	// avec ses effets, sa recette et sa panoplie associée.
	// Permet aussi d’ajouter l’équipement à une panoplie personnalisée.
	// ===============================================================
	 ...
	 * - `equipement` : objet complet de l’équipement sélectionné.
	 * - `panoplie` : panoplie officielle (issue des données statiques).
	 * - `panopliesDisponibles` : panoplies personnalisées (store utilisateur).
	 * - `emplacementChoisi` : position où placer l’équipement.
	 */
	...
	/**
	 * Chargement dynamique de l’équipement selon le paramètre de l’URL.
	 * 🔁 Appel répété après chaque navigation grâce à `afterNavigate`.
	 *
	 * ⚠️ Bug rencontré :
	 * - Certains équipements n’étaient pas trouvés car le nom dans l’URL
	 *   contenait des caractères encodés (accents, espaces).
	 * ✅ Solution : `decodeURIComponent($page.params.nom)` avant recherche.
	 */
	function chargerEquipement() {
		...
		/**
		 * Lorsqu’un équipement est chargé, on tente d’associer automatiquement
		 * sa panoplie officielle.
		 * ⚠️ Problème initial : la fonction `getPanoplieParEquipement`
		 *   échouait sur la casse ou les accents dans les noms d’items.
		 * ✅ Solution : ajout d’une normalisation côté service.
		 */
		if (equipement) {
	...
	/**
	 * ✅ Réactivité :
	 * - Les variables marquées `$:` sont recalculées automatiquement
	 *   quand leurs dépendances changent (`$page`, `$panopliesUtilisateur`).
	 * - Exemple : changement de route → rechargement automatique de l’équipement.
	 */
	afterNavigate(() => {
		chargerEquipement();
	});
	...
	/**
	 * Enregistre le prix personnalisé de l’équipement
	...
	/**
	 * Ajoute l’équipement courant à une panoplie personnalisée.
	 * Vérifie la sélection d’une panoplie et d’un emplacement.
	 *
	 * ⚠️ Bug observé :
	 * - L’ajout ne se mettait pas à jour immédiatement dans la vue.
	 * ✅ Solution : les stores réactifs ont été corrigés
	 *   pour créer une nouvelle référence d’objet (immutabilité).
	 */
	...
	<article class="fiche">
		...
		<section class="ajout-panoplie">
			...
			{#if panopliesDisponibles.length === 0}
				<p>
					Aucune panoplie enregistrée pour le moment. Rendez-vous dans la section « Gestion des
					panoplies » pour créer votre première configuration.
				</p>
			{:else}
				<div class="controle">
